...
const Header: React.FC<HeaderProps> = ({ onViewChange, activeView }) => {
  return (
    <header className="fixed top-0 left-0 w-full z-50 pointer-events-none">
      {/* Full-width transparent/glass bar background */}
      <div className="absolute inset-0 bg-white/40 backdrop-blur-xl border-b border-white/40 shadow-sm pointer-events-none" />
      ...
      <div className="relative max-w-7xl mx-auto py-3 md:py-5 px-4 md:px-8 flex justify-between items-center pointer-events-auto gap-3">
        {/* Brand Identity: F. App Icon + Wordmark */}
        <div 
          onClick={() => onViewChange('search')}
          className="flex items-center gap-2 md:gap-3 cursor-pointer group shrink-0"
        >
          {/* App Icon Square */}
          <div className="w-8 h-8 md:w-10 md:h-10 bg-gradient-to-br from-[#A8DADC] to-[#457B9D] text-white rounded-[0.7rem] md:rounded-[0.9rem] flex items-center justify-center shadow-md group-hover:scale-110 transition-transform relative overflow-hidden">
            <span className="text-sm md:text-xl font-black select-none font-sans">F</span>
        ...
        {/* Navigation - Enhanced spacing and active state */}
        <nav className="flex items-center gap-1 md:gap-2 bg-white/80 p-1 md:p-1.5 rounded-full border border-white/50 shadow-sm">
          {[
