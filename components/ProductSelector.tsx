    ...
    <div className="space-y-10 md:space-y-20 pb-32">
          ...
          <section key={category} className="space-y-8 px-4">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 md:gap-10">
                    ...
                    >
                      <div className="w-full aspect-square bg-slate-100 overflow-hidden relative border-b border-slate-100 shrink-0">
                         ...
                         <div className="absolute top-3 left-3 z-20 flex flex-col gap-1">
                            ...
                            <span className={`px-3 py-1 text-white rounded-full text-[7px] font-black tracking-widest uppercase flex items-center gap-1 ${product.isVerified ? 'bg-emerald-500' : conf.color}`}>
                      ...
                      <div className="p-6 flex-1 flex flex-col">
                        ...
                        <button 
                          ...
                          className={`mt-auto py-3 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all ${product.isVerified ? 'bg-slate-100 text-slate-400' : 'bg-slate-900 text-white shadow-lg hover:bg-blue-600'}`}
                        >
              ...
              <div onClick={() => setIsAdding(true)} className="group cursor-pointer rounded-[2rem] border-2 border-dashed border-slate-300 bg-white/40 hover:bg-white hover:border-blue-500 transition-all flex flex-col items-center justify-center h-[400px] md:h-[500px] p-8 text-center shadow-sm">
                ...
                <h4 className="mt-4 font-black text-slate-900 text-[10px] md:text-sm uppercase tracking-widest">Manual Injection</h4>
              </div>
            </div>
          </section>
        ))}
        ...
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-xl z-[250] flex items-center justify-center p-4">
           <div className="bg-white rounded-[3rem] w-full max-w-lg p-10 shadow-2xl animate-in zoom-in-95 duration-300">
             <div className="flex items-center gap-4 mb-8">
               ...
               <div>
                 <h3 className="text-xl font-black text-slate-900 uppercase">Verify Specifications</h3>
                 <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Correct AI estimates for project accuracy</p>
               </div>
             </div>
             <form onSubmit={handleVerifySubmit} className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                   <div className="space-y-1">
                     ...
                     <input type="number" required value={verifyingItem.quantity} onChange={e => setVerifyingItem({...verifyingItem, quantity: Number(e.target.value)})} className="w-full bg-slate-50 border border-slate-200 rounded-xl p-3 font-bold" />
                   </div>
                   <div className="space-y-1">
                     <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Unit</label>
                     <input required value={verifyingItem.suggestedUnit} onChange={e => setVerifyingItem({...verifyingItem, suggestedUnit: e.target.value})} className="w-full bg-slate-50 border border-slate-200 rounded-xl p-3 font-bold" />
                   </div>
                </div>
                <div className="space-y-1">
                  <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Confirmed Dimensions</label>
                  <input required value={verifyingItem.dimensions || ''} onChange={e => setVerifyingItem({...verifyingItem, dimensions: e.target.value})} className="w-full bg-slate-50 border border-slate-200 rounded-xl p-3 font-bold" placeholder="e.g. 1200 x 600 mm" />
                </div>
                <div className="space-y-1">
                  <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Description</label>
                  <textarea value={verifyingItem.description} onChange={e => setVerifyingItem({...verifyingItem, description: e.target.value})} className="w-full bg-slate-50 border border-slate-200 rounded-xl p-3 font-medium text-sm" rows={2} />
                </div>
                <div className="pt-4 flex gap-4">
                  <button type="button" onClick={() => setVerifyingItem(null)} className="flex-1 py-4 text-slate-400 font-black text-[10px] uppercase">Discard</button>
                  <button type="submit" className="flex-2 py-4 bg-slate-950 text-white rounded-xl font-black text-[10px] uppercase tracking-widest shadow-xl px-10">Confirm & Save</button>
                </div>
             </form>
           </div>
        </div>
      ...
      {isAdding && (
        <div className="fixed inset-0 bg-slate-950/70 backdrop-blur-md z-[200] flex items-center justify-center p-4">
          <div className="bg-white rounded-[3rem] w-full max-w-lg p-10 overflow-hidden shadow-2xl">
               <h3 className="text-2xl font-black text-slate-900 uppercase tracking-tight mb-8">Manual Injection</h3>
               <form onSubmit={handleManualSubmit} className="space-y-4">
                 <input autoFocus required value={newName} onChange={e => setNewName(e.target.value)} placeholder="Asset Name (e.g. Wooden Pedestal)" className="w-full bg-slate-50 border border-slate-200 rounded-xl py-4 px-6 font-bold" />
                 <input value={newDims} onChange={e => setNewDims(e.target.value)} placeholder="Dimensions" className="w-full bg-slate-50 border border-slate-200 rounded-xl py-4 px-6 font-bold" />
                 <textarea value={newDesc} onChange={e => setNewDesc(e.target.value)} placeholder="Detailed Description" className="w-full bg-slate-50 border border-slate-200 rounded-xl py-4 px-6 font-medium text-sm" rows={3} />
                 <div className="flex gap-4 pt-4">
                   ...
                   <button type="submit" className="flex-2 px-10 py-4 bg-slate-950 text-white rounded-xl font-black text-[10px] uppercase shadow-xl">Inject to Hub</button>
                 </div>
               </form>
          </div>
        </div>
      ...
      <div className="fixed bottom-6 left-1/2 -translate-x-1/2 w-[94%] max-w-5xl bg-white/95 backdrop-blur-3xl p-3 rounded-[3rem] border border-white shadow-[0_20px_60px_-15px_rgba(0,0,0,0.2)] z-[60] flex flex-col md:flex-row items-center gap-3">
        ...
        <div className="flex gap-3 w-full md:w-auto">
          <button
            ...
            `}
          >
            {needsVerification ? 'Action Required: Verify Low-Conf Items' : loading ? 'Neural Search...' : `Source ${selectedIds.size} Assets`}
          </button>
        </div>
      </div>
    </div>
